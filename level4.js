...
function mainLoop() {
    ...
    if(PB.playerAcc.magnitude() != 0) changed = true
    Objects.forEach((b, index) => {
        if (b.player) {
            ...
            if(!simulationStarted) {
                ctx.beginPath()
                ctx.arc(PB2.pos.x, PB2.pos.y, radius, 0, 2*Math.PI)
                ctx.lineWidth = 3
                ctx.strokeStyle = "green"
                ctx.stroke()

                b.pos.y = PB2.pos.y - radius
                b.pos.x = PB2.pos.x
                b.vel = new Vector(Math.sqrt(gravitationalConstant * PB2.mass / (radius*10**8)), 0)
            } else if(!changed) {
                ctx.beginPath()
                ctx.lineWidth = 3
                ctx.strokeStyle = "blue"
                ctx.arc(PB2.pos.x, PB2.pos.y, radius, -Math.PI/2, Math.atan2(b.pos.y - PB2.pos.y, b.pos.x - PB2.pos.y))
                ctx.stroke()
            } else {
        ...
        if(simulationStarted) b.calcPos()
        b.displayVectors()
    })
...
const radiusInput = document.getElementById("range-raggio")
...
function updateRadius() {
    ...
    radiusLabel.innerText = radius.toString()
}
